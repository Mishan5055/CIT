...
def GenerateGrid():
    ...
    a1b, a1l, a1h = SetUpGrid(n_b,n_l,n_h)
...
def ImportDataFromTomoI(country, year4, day, CODE, epoc: int) -> tuple[np.ndarray,list[XYZ],list[XYZ]]:
...
def Generate_C(country: str, year4: int, day: int, code: str, ep: int, lsat: list[XYZ], lrec: list[XYZ], a1b: np.ndarray, a1l: np.ndarray, a1h: np.ndarray, debug_mode=False) -> lil_matrix:
    n_b=len(a1b)-1
    n_l=len(a1l)-1
    n_h=len(a1h)-1
    n_all=n_b*n_l*n_h
...
def Del_Plasmaphic(tecs, recs, sats, H, year4, doy, epoch, a1h, a1b, a1l):
    UT = epoch/120.0
...
def Generate_Y(a1h,a1b,a1l, year4, doy, epoch):
    ...
    for jb in range(n_b):
        for kl in range(n_l):
            ...
            Y[idx1,0] = _interpolate(_dIRI, m_B, M_B, m_L,
                                   M_L, nb, nl, 0.5*(a1b[jb]+a1b[jb+1]), 0.5*(a1l[kl]+a1l[kl+1]))/1.0e+16
    return Y
...
def Tomography(A: csr_matrix, B: np.ndarray, H: csr_matrix, Y: np.ndarray, co: float, x_0:np.ndarray, First: bool) -> np.ndarray:
    ...
    if First:
        ...
        X,info = bicgstab(A.T*A+co*co*H.T*H, A.T*B+co*co*H.T*Y, x0=x_0, tol=1.0e-5,maxiter=1000)
...
def puts(B, country, year, day, CODE,  epoch):
    ...
    with open(tomob, "w") as bf:
        print(B.shape[1],file=bf)
        for i in range(B.shape[1]):
            print(i, B[0, i], file=bf)


def OutPut(X, op_file, epoch, a1h, a1b, a1l):
    n_h=len(a1h)-1
    n_b=len(a1b)-1
    n_l=len(a1l)-1
    # print(X.shape)
...
def process1(country:str, year4:int, day:int, code:str, ep:int, a1):
    a1h=a1[0]
    a1b=a1[1]
    a1l=a1[2]
    ep=int(ep)
    ...
    lil_A = Generate_C(
            country, year4, day, code, ep, sat, rec, a1b, a1l, a1h, debug_mode=False)
    B = Del_Plasmaphic(tec, rec, sat, np.max(a1h), year4, day, ep, a1h, a1b, a1l)
    puts(B, country, year4, day, code, ep)


def process2(exp: EXPERIMENT,a1h, a1b, a1l, csr_H, eps, idx, cof: float):
    country=exp.c
    year4=exp.y
    day=exp.d
    code=exp.code
    ep=exp.ep
    print(country,year4,day,code,ep,idx)
    csr_A = Read_A(exp)
    B = Read_B(exp)
    Y = Generate_Y(a1h, a1b, a1l, year4, day, ep)
    co = float(csr_matrix.trace(csr_A.T*csr_A)/csr_matrix.trace(csr_H.T*csr_H))
    if idx<=3:
        latest=0
    else:
        latest=idx-3
    print("latest :",eps[latest])
    exp_latest=EXPERIMENT(country, year4, day, code, eps[latest])
    X_0=Read_X(exp_latest)
    X=Tomography(csr_A, B, csr_H, Y, cof*co, X_0, First=False)
    os.makedirs("{dr}/tomo/{c}/{y:04}/{d:03}/{code}".format(
            dr=drive,c=country, y=year4, d=day, code=code), exist_ok=True)
    op_file = "{dr}/tomo/{c}/{y:04}/{d:03}/{code}/{epc:04d}.tomo".format(
            dr=drive,c=country, y=year4, d=day, code=code, epc=ep)
    OutPut(X, op_file, ep, a1h, a1b, a1l)
...
def __Tomography__(country: str, year4: int, day: int, CODE: str, eps: np.ndarray, cof: float, alpha: float):
    
    print("End prepare executors",time.time()-start)
    ...
    # prepare C,B for all epochs
    csr_H = Generate_H(a2b, a2l, a2h, alpha)
    trH_2=csr_matrix.trace(csr_H.T*csr_H)

    
    print("Set common condition",time.time()-start) # 3.7 sec
    
    with ProcessPoolExecutor(max_workers=2) as executor:
        futures=[executor.submit(process1,country,year4,day,CODE,ep,[a1h,a1b,a1l]) for ep in eps]
    
    # for ep in eps:
    #     process1(country,year4,day,CODE,ep)
        
    print("Set C",time.time()-start)
    
    
    exp=EXPERIMENT(country,year4,day,CODE,eps[0])
    # First epoch
    csr_A=Read_A(exp)
    csr_B=Read_B(exp)
    csr_Y=Generate_Y(a1h, a1b, a1l, year4, day, eps[0])
    co = float(csr_matrix.trace(csr_A.T*csr_A)/trH_2)

    print("Import 1st cond.",time.time()-start)
    
    X=Tomography(csr_A, csr_B, csr_H, csr_Y, cof*co,x_0=np.nan,First=True)
    
    print("end 1st calc",time.time()-start)
    
    os.makedirs("{dr}/tomo/{c}/{y:04}/{d:03}/{code}".format(
        dr=drive,c=country, y=year4, d=day, code=CODE), exist_ok=True)
    op_file = "{dr}/tomo/{c}/{y:04}/{d:03}/{code}/{epc:04d}.tomo".format(
            dr=drive,c=country, y=year4, d=day, code=CODE, epc=eps[0])
    OutPut(X, op_file, eps[0],a1h,a1b,a1l)
    print("Export 1st result",time.time()-start)
    ...
    with ProcessPoolExecutor(max_workers=2) as executor:
        futures=[executor.submit(process2,exp,a1h,a1b,a1l,csr_H,eps,i,cof) for i,exp in enumerate(exps)]
...
if __name__=="__main__":
    ...
    year4=2016
    day=107
    code="keq_1+1_08d_3_1_3_3-1"
    eps=np.arange(1550,2220,1)
    cof=1.0e+1
    alpha=0.3
    
    start=time.time()
    __Tomography__(country,year4,day,code,eps,cof,alpha)